...
# byebug
while Rails.configuration.chatkit.get_rooms()[:body].count > 0 do
  Rails.configuration.chatkit.get_rooms()[:body].map do |room|
    Rails.configuration.chatkit.delete_room({id: room[:id]})
  end
end

# delete user account from live chat kit
while Rails.configuration.chatkit.get_users()[:body].count > 0 do
  Rails.configuration.chatkit.get_users()[:body].map do |user|
    Rails.configuration.chatkit.delete_user({id: user[:id]})
  end
end
...
resorts.sample(300).each do |resort|
...
100.times do
  ...
  Event.create(title: titles.sample, description: descriptions.sample, date: Faker::Date.forward(90), image_url: image_urls.sample, resort_id: Resort.pluck(:id).sample, host_id: User.pluck(:id).sample)
end
